...
This file is pulled from AWS Kinesis Firehose CloudWatch Logs Processor blueprint mentioned here:
...
exports.handler = (event, context, callback) => {
    Promise.all(event.records.map(r => {
        ...
        if (data.messageType === 'CONTROL_MESSAGE') {
            ...
            // NB. transformed can have multiple records. It is a collection of promises.
            const promises = data.logEvents.map(transformLogEvent);
            return Promise.all(promises)
                .then(transformed => {
									  const payload = transformed.filter(flowLog => flowLog != 'DROP').join('\n');
